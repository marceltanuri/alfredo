...
function buildReport(json) {
    ...
    let _innerHTML = `Saldo Total: <span class="totalBalance">${json.totals.balances.sum}</span>`
    _innerHTML += "<br/>"
    ...
    for (const source of json.totals.balances.sources) {
        _innerHTML += `Saldo ${source.name}: <span class="${source.name}">${source.total}</span>`
        _innerHTML += "<br/>"
    }
...
function drawChart() {
    ...
    options.slices[parseInt(chartData.length) - 2] = {
        offset: 0.2,
        color: "#6495ed"
    }
